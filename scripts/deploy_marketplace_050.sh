...
test "${CONFIRM_MARKETPLACE_050_DEPLOY:-false}" = true || {
  echo "CONFIRM_MARKETPLACE_050_DEPLOY=true required" >&2
  exit 2
}
test -f .env
...
rollback() {
  ...
  if [ "$complete" != true ] && [ "$switched" = true ]; then
    ...
    resume_service || true
    echo "marketplace_050_deploy=FAIL rollback_previous_production=ATTEMPTED" >&2
...
$DOCKER compose exec -T onecent-db psql -U onecent -d onecent -v ON_ERROR_STOP=1 \
  -c "UPDATE service_settings SET value='false',updated_at=now(),updated_by='marketplace-050-deploy' WHERE key='service_enabled'" >/dev/null
paused=true
...
complete=true
trap - EXIT INT TERM
echo "marketplace_050_deploy=PASS; successful_settlements_unchanged=$after_payments"
