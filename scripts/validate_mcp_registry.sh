...
$PYTHON -c 'import json; d=json.load(open("server.json",encoding="utf-8")); assert d["name"]=="ru.maxzoa/1cent"; assert d["version"]=="0.5.0"; assert d["remotes"]==[{"type":"streamable-http","url":"https://1cent.maxzoa.ru/mcp"}]; print("remote metadata=PASS; version=0.5.0")'
