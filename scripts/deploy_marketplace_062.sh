...
test "${CONFIRM_MARKETPLACE_062_DEPLOY:-false}" = true || {
  echo "CONFIRM_MARKETPLACE_062_DEPLOY=true required" >&2
  exit 2
}
test -f .env
...
resume_service() {
  $DOCKER compose exec -T onecent-db psql -U onecent -d onecent -v ON_ERROR_STOP=1 \
    -c "UPDATE service_settings SET value='true',updated_at=now(),updated_by='marketplace-062-deploy' WHERE key='service_enabled'" >/dev/null
  paused=false
}
...
rollback() {
  ...
  if [ "$complete" != true ] && [ "$switched" = true ]; then
    ...
    resume_service || true
    echo "marketplace_062_deploy=FAIL rollback_previous_production=ATTEMPTED" >&2
...
umask 077
cp .env .env.production.marketplace-062.saved
chmod 600 .env.production.marketplace-062.saved
...
$DOCKER compose exec -T onecent-db psql -U onecent -d onecent -v ON_ERROR_STOP=1 \
  -c "UPDATE service_settings SET value='false',updated_at=now(),updated_by='marketplace-062-deploy' WHERE key='service_enabled'" >/dev/null
paused=true
...
curl -fsS http://127.0.0.1:18013/status.json | grep -q '"version":"0.6.2"'
rm -f "$MAINTENANCE_FILE"
...
complete=true
trap - EXIT INT TERM
echo "marketplace_062_deploy=PASS; successful_settlements_unchanged=$after_payments"
